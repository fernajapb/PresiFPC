...
function simularTorneoB(semestre) {
 if (!equipoUsuario) {
  ...
  agregarNotificacion(`🎉 ¡Felicidades!, La DIMAYOR felicita al Club: ${campeon} por ser Campeón de la Primera B - ${temporadaActual}-${semestre}`);
  agregarTitulo(campeon, "B");
...
function simularFinalB(finalistas) {
  ...
  if (campeon === equipoUsuario) {
    primerabGanadasPorUsuario++;
    document.getElementById("titulospb").innerText = `Primera B: ${primerabGanadasPorUsuario}`;
  }
