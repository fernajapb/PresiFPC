...
function simularPartido(equipoA, equipoB) {
  ...
  const ventaja = Math.tanh(diferencia / 25);
...
function elegirGoleadorPorPeso(plantilla) {
  ...
  for (let jugador of plantilla) {
    const posicion = jugador.posicion?.toLowerCase() || "MCO";
...
function simularPlayoffs() {
  ...
  try { simularMercadoFichajesNuevo() ;} catch(e) {}
...
function simularPlayoffsMX(clasificados) {
  ...
  try { actualizarBuzon(); evaluarMoralHinchada(); evaluarObjetivos(); actualizarFuerzaUI(); simularMercadoFichajesNuevo() ;} catch (e) {}
}
